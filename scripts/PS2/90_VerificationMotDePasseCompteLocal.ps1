...
try {
    ...
    $ExecutionMode = Get-CGlobalExecutionMode
    if ($ExecutionMode -ne 'Interactif') {
        $PasswordAction = [string](Get-CGlobalDecision -Key 'LocalPasswordAction' -Default 'No')
        $PasswordBlob = [string](Get-CGlobalDecision -Key 'LocalPasswordBlob' -Default '')
        if ($PasswordAction -ne 'Yes' -or [string]::IsNullOrWhiteSpace($PasswordBlob)) {
            Write-Log "Definition du mot de passe ignoree en mode $ExecutionMode" "WARN"
            exit 0
        }
        try {
            $SecurePassword = ConvertTo-SecureString -String $PasswordBlob -ErrorAction Stop
            Set-LocalUser -Name $CurrentUserName -Password $SecurePassword -ErrorAction Stop
            Write-Log "Mot de passe defini avec succes depuis la prevalidation" "OK"
            exit 0
        }
        catch {
            Write-Log "Echec de l'application du mot de passe prevalide : $($_.Exception.Message)" "ERROR"
            exit 1
        }
    }
